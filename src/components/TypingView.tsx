...
export function TypingView({ snapshot, inputRef, onKeyDown, onFocusClick }: TypingViewProps) {
  ...
  return (
    <div className="relative" onClick={onFocusClick}>
      <input
        ref={inputRef}
      ...
      <div
        ref={scrollerRef}
        className="h-[7.2rem] overflow-hidden font-mono text-[1.7rem] leading-[2.4rem] tracking-wide"
      >
...
function Word({
  ...
  return (
    <span
      ref={wordRef}
      className={cn('relative whitespace-nowrap', active && 'text-ink')}
    >
      {word.chars.map((cell, i) => (
        <span key={i}>
          ...
          <span
            className={cn(
              cell.flag === 'pending' && (active ? 'text-ink/55' : 'text-muted'),
              cell.flag === 'correct' && 'text-ink',
              cell.flag === 'incorrect' && 'text-error',
            )}
          >
            {cell.expected}
          </span>
